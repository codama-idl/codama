...
test('it exposes the Codama error context', () => {
    const error = new KinobiError(CODAMA_ERROR__UNRECOGNIZED_NODE_KIND, { kind: 'missingNode' });
...
test('it exposes the cause when provided', () => {
    ...
    const error = new KinobiError(CODAMA_ERROR__UNRECOGNIZED_NODE_KIND, { cause, kind: 'missingNode' });
...
test('it returns `true` when the error code matches', () => {
    const error = new KinobiError(CODAMA_ERROR__UNRECOGNIZED_NODE_KIND, { kind: 'missingNode' });
    expect(isKinobiError(error, CODAMA_ERROR__UNRECOGNIZED_NODE_KIND)).toBe(true);
});

test('it returns `false` when the error code does not match', () => {
    const error = new KinobiError(CODAMA_ERROR__UNRECOGNIZED_NODE_KIND, { kind: 'missingNode' });
    expect(isKinobiError(error, CODAMA_ERROR__UNEXPECTED_NODE_KIND)).toBe(false);
});
