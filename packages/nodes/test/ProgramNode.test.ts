...
test('it returns the right node kind', () => {
    const node = programNode({ identifier: 'foo', publicKey: '1111' });
...
test('it defaults the version to 0.0.0', () => {
    const node = programNode({ identifier: 'foo', publicKey: '1111' });
    expect(node.version).toBe('0.0.0');
});

test('it does not carry an origin', () => {
    const node = programNode({ identifier: 'foo', publicKey: '1111' });
    expect('origin' in node).toBe(false);
});
...
test('it omits an array attribute even when an explicit empty array is passed', () => {
    const node = programNode({ accounts: [], identifier: 'foo', publicKey: '1111' });
    expect('accounts' in node).toBe(false);
});

test('it keeps array attributes when they are non-empty', () => {
    const account = accountNode({ data: structTypeNode([]), identifier: 'myAccount' });
    const node = programNode({ accounts: [account], identifier: 'foo', publicKey: '1111' });
