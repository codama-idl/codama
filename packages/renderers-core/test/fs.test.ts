...
if (__NODEJS__) {
    ...
    test('it fails to call readJson', () => {
        expect(() => readJson('./path')).toThrow(
            new KinobiError(CODAMA_ERROR__NODE_FILESYSTEM_FUNCTION_UNAVAILABLE, { fsFunction: 'readFileSync' }),
        );
    });
    test('it fails to call createDirectory', () => {
        expect(() => createDirectory('./path')).toThrow(
            new KinobiError(CODAMA_ERROR__NODE_FILESYSTEM_FUNCTION_UNAVAILABLE, { fsFunction: 'mkdirSync' }),
        );
    });
    test('it fails to call deleteDirectory', () => {
        expect(() => deleteDirectory('./path')).toThrow(
            new KinobiError(CODAMA_ERROR__NODE_FILESYSTEM_FUNCTION_UNAVAILABLE, { fsFunction: 'rmSync' }),
        );
    });
    test('it fails to call createFile', () => {
        expect(() => createFile('./path', 'content')).toThrow(
            new KinobiError(CODAMA_ERROR__NODE_FILESYSTEM_FUNCTION_UNAVAILABLE, { fsFunction: 'writeFileSync' }),
        );
    });
}
