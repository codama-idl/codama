...
type DefaultValueEncoderSupportedNodeKind =
  (typeof DEFAULT_VALUE_ENCODER_SUPPORTED_NODE_KINDS)[number];
...
export function createDefaultValueEncoderVisitor(codec: {
  ...
  return {
    ...
    visitBytesValue: (node: BytesValueNode) =>
      codec.encode([node.encoding, node.data]),
    visitEnumValue: (node: EnumValueNode) =>
      codec.encode({ __kind: pascalCase(node.variant) }),
