...
describe("enumTypeNode", () => {
  ...
  test("should transform bytes in tuple variant", () => {
    const enumWithTupleVariant = enumTypeNode([
      ...
      enumTupleVariantTypeNode(
        "withBytes",
        tupleTypeNode([bytesTypeNode(), numberTypeNode("u8")]),
      ),
    ]);

    const transformer = createCodecInputTransformer(
      enumWithTupleVariant,
      rootNodeMock,
      {
        bytesEncoding: "base16",
      },
  ...
  test("should throw on unknown variant", () => {
    const enumWithVariants = enumTypeNode([
      enumEmptyVariantTypeNode("known1"),
      enumEmptyVariantTypeNode("known2"),
    ]);
    ...
    expect(() => transformer(input)).toThrow(
      /Expected \[one of \[known1, known2\]\] for \[enumTypeNode\]/,
    );
  });

  test("should pass through non-object input for enumTypeNode", () => {
    const enumNode = enumTypeNode([
      enumEmptyVariantTypeNode("variant1"),
      enumEmptyVariantTypeNode("variant2"),
    ]);
  ...
  test("should handle nested enum with multiple bytes fields in struct variant", () => {
    ...
    const transformer = createCodecInputTransformer(complexEnum, rootNodeMock, {
      bytesEncoding: "base16",
    });
  ...
  test("should throw on tuple variant without fields", () => {
    ...
    expect(() => transformer(input)).toThrow(
      /Expected \[array \(fields\)\] for \[enumTupleVariantTypeNode\]/,
    );
  });
    ...
    const outerEnum = enumTypeNode([
      enumStructVariantTypeNode(
        ...
        structTypeNode([
          structFieldTypeNode({
            name: "inner",
            type: definedTypeLinkNode("InnerEnum"),
          }),
        ]),
      ),
    ]);

    const transformer = createCodecInputTransformer(outerEnum, root, {
      bytesEncoding: "base16",
    });
