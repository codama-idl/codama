import { integerTypeNode, ProgramNode, publicKeyTypeNode, rootNode, tupleTypeNode } from '@codama/nodes';
...
test('it can create rootNode only visitors that return new rootNode instances', () => {
    ...
    const visitor = rootNodeVisitor(node =>
        rootNode(node.program, {
            additionalPrograms: [...(node.additionalPrograms ?? []), {} as ProgramNode],
        }),
    );
