...
describe('formatAmountValue', () => {
    ...
    test('it returns null for a non-integer value rather than throwing', async () => {
        // Given an amount display and a non-integer value that cannot be scaled as a fixed-point integer.
        const node = amountNumberDisplayNode({ decimals: numberValueNode(2) });

        // When we format the amount.
        const result = await formatAmountValue(1.5, node, context());
