...
test('it accepts IDLs sharing the spec major and returns them unchanged', () => {
    ...
    const oldest = { ...rootNode(program), version: '2.0.0' as CodamaVersion };
...
test('it rejects IDLs from another spec major', () => {
    ...
    const older = { ...rootNode(program), version: '1.3.0' as CodamaVersion };
    ...
    expect(() => visit(older, checkCodamaVersionVisitor())).toThrow(
        new CodamaError(CODAMA_ERROR__VERSION_MISMATCH, { codamaVersion: CODAMA_VERSION, rootVersion: '1.3.0' }),
    );
});
