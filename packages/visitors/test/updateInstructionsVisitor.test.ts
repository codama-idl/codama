...
test('it updates the name of an instruction', () => {
    ...
    expect((result.instructions ?? [])[0].name).toBe('myNewInstruction' as IdentifierString);
});
...
test('it updates the name of an instruction within a specific program', () => {
    ...
    expect((result.program.instructions ?? [])[0].name).toBe('newTransfer' as IdentifierString);

    // But not the second instruction.
    expect(((result.additionalPrograms ?? [])[0].instructions ?? [])[0].name).toBe('transfer' as IdentifierString);
});
...
test('it updates the name of an instruction argument', () => {
    ...
    // Then we expect the following tree changes.
    assertIsNode(result, 'instructionNode');
    expect((result.arguments ?? [])[0].name).toBe('myNewArgument' as IdentifierString);
});
